  ...
  const handleQRScan = async (scanData) => {
    requireAuth(async () => {
      try {
        ...
        const response = await attendanceApi.recordScan(JSON.stringify(scanData));
        ...
        if (response.success) {
          ...
          // 成功メッセージを表示
          const actionText = response.data.action === 'checkin' ? '出席' : '退出';
          alert(`${actionText}記録が完了しました！\nグループ: ${scanData.groupName}\n時刻: ${new Date().toLocaleString('ja-JP')}`);
        } else {
          setError(response.message || '出欠記録の送信に失敗しました');
        }
      } catch (err) {
